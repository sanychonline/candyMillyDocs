(() => {
  ...
  function apply(locale, updateUrl = true) {
    ...
    document.querySelectorAll(".brand span, .final-game-cta .game-kicker").forEach((element) => {
      element.textContent = brandLabels[locale] || brandLabels.en;
    });
    document.querySelectorAll(".footer .footer-inner > span").forEach((element) => {
      element.textContent = `© 2026 ${brandLabels[locale] || brandLabels.en}`;
    });
